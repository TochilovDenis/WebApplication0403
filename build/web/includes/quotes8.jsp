...
<%
    ...
    if (session.getAttribute("userAddedQuotes") != null) {
        ...
        for (String category : quotes.keySet()) {
            ...
            for (int i = 0; i < quotes.get(category).length; i++) {
                userAdded.add(true); // По умолчанию нельзя редактировать/удалять
            }
    ...
    String deleteQuote = request.getParameter("deleteQuote");
    ...
    if (addQuote != null && newQuote != null && !newQuote.trim().isEmpty()
        ...
        response.sendRedirect("quotes8.jsp" + (about != null ? "?about=" + about : ""));
        return;
    }
    ...
    // Удаление цитаты (БЕЗ ПОДТВЕРЖДЕНИЯ - СРАЗУ)
    if (deleteQuote != null && "true".equals(deleteQuote) && deleteIndex != null && deleteCategory != null) {
        int index = Integer.parseInt(deleteIndex);
        String category = deleteCategory;
        
        // Получаем текущие цитаты и флаги пользовательских
        String[] categoryQuotes = quotes.get(category);
        List<Boolean> userAdded = userAddedQuotes.get(category);
        
        if (categoryQuotes != null && index >= 0 && index < categoryQuotes.length) {
            
            // Проверяем, можно ли удалить (только пользовательские)
            if (userAdded != null && index < userAdded.size() && userAdded.get(index) == true) {
                
                // Создаем новый массив без удаляемой цитаты
                String[] updatedQuotes = new String[categoryQuotes.length - 1];
                List<Boolean> updatedUserAdded = new ArrayList<Boolean>();
                
                int newIndex = 0;
                for (int i = 0; i < categoryQuotes.length; i++) {
                    if (i != index) {
                        updatedQuotes[newIndex] = categoryQuotes[i];
                        if (userAdded != null && i < userAdded.size()) {
                            updatedUserAdded.add(userAdded.get(i));
                        }
                        newIndex++;
                    }
                }
                ...
                if (updatedQuotes.length > 0) {
                    quotes.put(category, updatedQuotes);
                    userAddedQuotes.put(category, updatedUserAdded);
                } else {
                    // Если цитат не осталось, удаляем категорию
                    quotes.remove(category);
                    userAddedQuotes.remove(category);
                }
                
                // Сохраняем в сессию
                session.setAttribute("quotes", quotes);
                session.setAttribute("userAddedQuotes", userAddedQuotes);
                session.setAttribute("message", "Цитата успешно удалена!");
            } else {
                session.setAttribute("message", "Нельзя удалить исходную цитату!");
            }
...
<html>
    ...
    <body>
        ...
        <div class="menu">
            <h3>Выберите тему:</h3>
            <a href="quotes8.jsp?about=Про образование" <%= "Про образование".equals(about) ? "class='active'" : ""%>>Образование</a>
            <a href="quotes8.jsp?about=Про личное развитие" <%= "Про личное развитие".equals(about) ? "class='active'" : ""%>>Личное развитие</a>
            <a href="quotes8.jsp?about=Про отношения" <%= "Про отношения".equals(about) ? "class='active'" : ""%>>Отношения</a>
            <a href="quotes8.jsp?about=Про история" <%= "Про история".equals(about) ? "class='active'" : ""%>>История</a>
        </div>

        <div class="search-form">
            <form method="get" action="quotes8.jsp">
                ...
                <a href="quotes8.jsp" style="margin-left: 10px; color: #dc3545;">Сбросить поиск</a>
                <% } %>
            </form>
        </div>
        ...
        <div class="edit-quote-form">
            ...
            <form method="post" action="quotes8.jsp">
                ...
                <a href="quotes8.jsp<%= about != null ? "?about=" + about : ""%>" class="btn-secondary">Отмена</a>
            </form>
        </div>
        <%
                }
                        ...
                        if (categoryQuotes != null) {
                            for (int i = 0; i < categoryQuotes.length; i++) {
                                ...
                                if (quote != null && quote.toLowerCase().contains(searchTerm)) {
                                    ...
                                    // Добавляем кнопки для пользовательских цитат
                                    if (isUserAdded) {
                                        out.println("<div class='quote-actions'>");
                                        out.println("<a href='quotes8.jsp?editQuote=true&editIndex=" + i + "&editCategory=" + category + (about != null ? "&about=" + about : "") + "' class='edit-btn btn-sm'>✏️ Редактировать</a>");
                                        out.println("<a href='quotes8.jsp?deleteQuote=true&deleteIndex=" + i + "&deleteCategory=" + category + (about != null ? "&about=" + about : "") + "' class='btn-danger-sm' onclick='return confirm(\"Вы уверены, что хотите удалить эту цитату?\")'>🗑️ Удалить</a>");
                                        out.println("</div>");
                                    }
                    ...
                    if (quotes_ != null && quotes_.length > 0) {
                        ...
                        for (int i = 0; i < quotes_.length; i++) {
                            if (quotes_[i] != null) {
                                ...
                                if (isUserAdded) {
                                    out.println("<div class='quote-actions'>");
                                    out.println("<a href='quotes8.jsp?editQuote=true&editIndex=" + i + "&editCategory=" + about + "' class='edit-btn btn-sm'>✏️ Редактировать</a>");
                                    out.println("<a href='quotes8.jsp?deleteQuote=true&deleteIndex=" + i + "&deleteCategory=" + about + "' class='btn-danger-sm' onclick='return confirm(\"Вы уверены, что хотите удалить эту цитату?\")'>🗑️ Удалить</a>");
                                    out.println("</div>");
                                }
                        ...
                        if (categoryQuotes != null) {
                            for (int i = 0; i < categoryQuotes.length; i++) {
                                if (categoryQuotes[i] != null) {
                                    ...
                                    if (isUserAdded) {
                                        out.println("<div class='quote-actions'>");
                                        out.println("<a href='quotes8.jsp?editQuote=true&editIndex=" + i + "&editCategory=" + category + "' class='edit-btn btn-sm'>✏️ Редактировать</a>");
                                        out.println("<a href='quotes8.jsp?deleteQuote=true&deleteIndex=" + i + "&deleteCategory=" + category + "' class='btn-danger-sm' onclick='return confirm(\"Вы уверены, что хотите удалить эту цитату?\")'>🗑️ Удалить</a>");
                                        out.println("</div>");
                                    }
...
<!--Задание 8
Добавьте к шестому заданию возможность удаления пользовательской цитаты.
